...
Create a sandbox pointed at the repository. This remote example requires the
`cloudsmith-repo-v0.1.0` tag to exist in [Releases](https://github.com/cloudsmith-io/cloudsmith-sbx-kits/releases);
use the local example before the first release:

```console
sbx run claude --kit "git+https://github.com/cloudsmith-io/cloudsmith-sbx-kits.git#ref=cloudsmith-repo-v0.1.0&dir=cloudsmith-repo" --kit-arg cloudsmith-repo.path=/acme/prod .
```
...
- The kit owns `~/.cargo/config.toml`, `~/.m2/settings.xml`, `~/.nuget/NuGet/NuGet.Config` and `~/.yarnrc.yml`; bundled files replace existing content when applied. It also sets `registry` in the agent's `~/.npmrc`, preserving other entries.
- Use sbx 0.42.x (compatibility baseline: 0.42.1). Docker Engine alone is not the sandbox runtime.
- Under organization governance, ask your administrator to allow the configured Cloudsmith hosts; kit allow rules cannot grant that access.
...
The default `path` is Cloudsmith's public CLI repository, so the kit validates and installs with no inputs. It is a smoke-test default, not a general-purpose dependency mirror. Set it explicitly for your project.
...
download domain even when metadata was requested from `*.cloudsmith.io`.
...
- **Yarn Berry**: does not automatically use `HTTPS_PROXY`. The bundled `~/.yarnrc.yml` maps Yarn's proxy settings to the sandbox-managed environment variables. Project settings that override these proxy values can prevent credential injection.
...
- **Docker**: pull only, using Cloudsmith-qualified image names. The CLI kit's API credential is injected only at `api.cloudsmith.io`; it does not enable `docker push` or registry login.
...
`permissions.network.allow` declares the format hosts and the redirect host. The end-to-end harness
checks sandbox creation under `deny-all`; that does not exercise every format
or prove all custom-domain redirects work. Use the format checks above with
your repository and inspect `sbx policy log <sandbox>` before adopting a new
...
Other kits may allow them. `dl.cloudsmith.io` is shared by many Cloudsmith
repositories; host-level policy cannot narrow it to one.
...
| Probe 401 | No working token reached Cloudsmith, or the binding was not approved | Run `sbx secret set cloudsmith-entitlement-token` interactively on the host, approve the binding, then recreate |
...
Only remove the host secret if no other sandbox needs it. Config files inside
the sandbox disappear with `sbx rm <sandbox>`; files in your mounted project
remain. Removing the sandbox or secret does not revoke the token in
Cloudsmith. Revoke or rotate it when retiring a sandbox that ran untrusted code.
