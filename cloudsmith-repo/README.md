...
`permissions.network.allow` is the kit's complete outbound contract. The end-to-end tests run under the `deny-all` policy, so every entry below was proved necessary and the list was proved complete.
