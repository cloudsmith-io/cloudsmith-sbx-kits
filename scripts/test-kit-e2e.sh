...
#   APP_NAME  scoped app name. Keep it in sync with tck/e2e_test.go, and keep
...
Each row under 'Blocked requests' is a host the kit reached for. Add it to
permissions.network.allow in the kit's spec.yaml, then run this script again.
EOF
  fi
  rm -f "$sbx_name_log"
  exit "$rc"
