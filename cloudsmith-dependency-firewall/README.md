...
Inside the sandbox, the proxy blocks a public registry and the same install works through Cloudsmith:
...
`sbx kit add` does not support deny rules, so recreate an existing sandbox to
add this kit. Organization governance can restrict access further and does not
remove these denies.

## How it works

Configure the repository and its authentication through `cloudsmith-repo` using the [usage examples](#usage). The flow below shows the named registry hosts this kit denies and the Cloudsmith hosts the composed policy allows; other hosts remain subject to that policy.
...
The kit denies Docker Hub too, so `docker run alpine` inside the sandbox fails unless you mirror the image in Cloudsmith, and kits whose startup pulls from Docker Hub (for example `qemu`) break.
...
- Kits that pull images from Docker Hub (`qemu`, `openclaw`, `nanoclaw`) break. The kit does not redirect image names; mirror the images in Cloudsmith or leave the firewall out.
...
Nothing on the host. The deny rules disappear with `sbx rm <sandbox>`. Remove rules that you added with `sbx policy deny network` using `sbx policy rm`.
