...
`deny-all`, but only exercise the requests those tests make, not every
...
Kits do not replace Docker's sandbox isolation or protect the files you mount
into a sandbox. Inspect kit content and credential bindings before you run a
kit. A kit source allowlist permits downloads from that source; it is not
...
Other sandboxes that bind the same service reuse the global service secret.
Removing a sandbox does not revoke its token in Cloudsmith or
